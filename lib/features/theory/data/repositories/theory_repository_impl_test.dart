...
void main() {
  group('TheoryRepositoryImpl', () {
    ...
    // Test 1: Vérifier qu'on récupère les catégories
    test('getCategories() retourne 7 catégories', () async {
      final categories = await repository.getCategories();

      expect(categories.length, equals(7)); // 0 + 6 catégories
      expect(categories.first.name, equals('Tous les mots'));
      expect(categories.last.name, equals('Famille'));
    });

    // Test 2: Vérifier qu'on récupère les bons mots par catégorie
    ...
    // Test 3: Vérifier le détail d'un mot
    test('getWordDetail() retourne les infos complètes du mot', () async {
      ...
      expect(wordDetail.exampleSentence, contains('(na-neun haksaeng-imnida)'));
    });
  });
}
